...
url = 'https://www.alphavantage.co/query?function=ETF_PROFILE&symbol=SPY&apikey=XHXULSW0RDYPBCNV'
r = requests.get(url)
datos = r.json()
...
for symbol in primeras_cinco_empresas:
    url_1 = f'https://www.alphavantage.co/query?function=OVERVIEW&symbol={symbol}&apikey=XHXULSW0RDYPBCNV'
    r_empresa = requests.get(url_1)
    url_2 = f'https://www.alphavantage.co/query?function=BALANCE_SHEET&symbol={symbol}&apikey=XHXULSW0RDYPBCNV'
    balance_sheet = requests.get(url_2)
    url_3 = f'https://www.alphavantage.co/query?function=INCOME_STATEMENT&symbol={symbol}&apikey=XHXULSW0RDYPBCNV'
    income_statement = requests.get(url_3)
    url_4 = f'https://www.alphavantage.co/query?function=CASH_FLOW&symbol={symbol}&apikey=XHXULSW0RDYPBCNV'
